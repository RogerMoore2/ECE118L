/***************************************************************************************
 *  File Name      : address_map_arm.s
 *  Project        : DE1-SoC / Cyclone V ARM Cortex-A9 Embedded System
 *  Description    : 
 *      This file defines symbolic constants for memory-mapped I/O and peripheral 
 *      device base and end addresses used in the Cyclone V SoC (DE1-SoC) system.
 *      It includes address values for:
 *          - DDR, On-chip, SDRAM, FPGA On-chip, and Character Buffer memory
 *          - FPGA peripheral devices (LEDs, HEX displays, switches, keys, JP connectors, etc.)
 *          - HPS (Hard Processor System) devices (GPIO, I2C, Timers, etc.)
 *          - ARM A9 MPCORE devices (private timer, interrupt controller, etc.)
 *      These definitions allow symbolic naming of addresses in assembly code,
 *      improving readability, maintainability, and portability of the source code.
 *
 ...
 *
 *  Usage Notes    :
 *      - All addresses are defined using the `.equ` directive to assign constant values.
 *      - Use these symbolic names in load/store instructions and peripheral 
 ...
 *
 *  Revision History:
 *      v1.0  - Initial version; defined complete address map for memory and peripherals.
 *
 ***************************************************************************************/
...
.equ PERIPH_BASE, 0xFFFEC000 /* base address of peripheral
devices */
...
*/
.equ ICCPMR, 0x04 /* interrupt priority mask
register */
...
*/
.equ ICDICER, 0x180 /* interrupt clear-enable
registers */
.equ ICDIPTR, 0x800 /* interrupt processor targets
registers */
.equ ICDICFR, 0xC00 /* interrupt configuration
registers */
